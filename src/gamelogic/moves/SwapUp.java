...
/**
 * Extends Swap class to implement the functionality to swap the current cell with the one above
 * it.
 */
public class SwapUp extends Swap {
   ...
   */
  public SwapUp(List<Boundary> boundaries) {
    super(boundaries);
  }

  /**
   * Returns the correct coordinates for the spot we want to swap the cell into above it.
   *
  ...
  public Coordinate getLocationToSwapWith(Agent currentAgent) {
    return currentAgent.getDirectUpToBody().get(FIRST_CELL_IN_BODY);
  }
