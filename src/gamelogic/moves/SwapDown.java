...
/**
 * Extends Swap class to implement the functionality to swap the current cell with the one below it.
 */
public class SwapDown extends Swap {
   ...
   */
  public SwapDown(List<Boundary> boundaries) {
    super(boundaries);
  }

  /**
   * Returns the correct coordinates for the spot we want to swap the cell into below it.
   *
  ...
  public Coordinate getLocationToSwapWith(Agent currentAgent) {
    return currentAgent.getDirectDownToBody().get(FIRST_CELL_IN_BODY);
  }
