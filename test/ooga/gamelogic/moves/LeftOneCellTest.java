...
class LeftOneCellTest {
  ...
  public void setUp() {
    ...
    curEntity =
        List.of(new Agent(List.of(grid.getCellFromCoordinate(new Coordinate(1, 2)),
            grid.getCellFromCoordinate(new Coordinate(0, 3)),
            grid.getCellFromCoordinate(new Coordinate(0, 2)))));
  }

  @Test
  void moveActiveAgentOneSpaceLeft() {
    int expected = 3;
    Move m = new LeftOneCell(boundaries);
  ...
  @Test
  void moveActiveAgentLeftOneSpotCheckOpen() {
    int expected = 0;
    Move m = new LeftOneCell(boundaries);
    m.moveEntity(curEntity, grid);
