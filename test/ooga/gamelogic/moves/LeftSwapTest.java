...
class LeftSwapTest {
  ...
  void setUp() {
    grid = new Grid(10, 10);
    grid.changeCell(new Coordinate(0, 1), 5);
    grid.changeCell(new Coordinate(0, 2), 3);
    grid.updateCells();
    curEntity = List.of(new Agent(List.of(grid.getCell(0, 2))));
  ...
  void singleSwapTest() {
    ...
    Move m = new SwapLeft(boundaries);
    m.moveEntity(curEntity, grid);
    int stateSecond = grid.getCellFromCoordinate(new Coordinate(0, 2)).getCurrentState();
    int stateFirst = grid.getCellFromCoordinate(new Coordinate(0, 1)).getCurrentState();
