...
class DownOneCellTest {
  ...
  public void setUp() {
    ...
    curEntity =
        List.of(new Agent(List.of(grid.getCellFromCoordinate(new Coordinate(1, 2)),
            grid.getCellFromCoordinate(new Coordinate(0, 1)),
            grid.getCellFromCoordinate(new Coordinate(0, 2)))));
  }

  @Test
  void moveActiveAgentDownOneStep() {
    int expected = 3;
    Move m = new DownOneCell(boundaries);
    m.moveEntity(curEntity, grid);
    int state = grid.getCellFromCoordinate(new Coordinate(1, 1)).getCurrentState();
  ...
  void moveActiveAgentDownTwice() {
    ...
    m.moveEntity(curEntity, grid);
    m.moveEntity(curEntity, grid);
    int state = grid.getCellFromCoordinate(new Coordinate(3, 2)).getCurrentState();
    assertEquals(expected, state);
  }
}